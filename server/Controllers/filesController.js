...
const getAllDocs = async (req, res) => {
  try {
    await Document.find(
      { user_id: req.user.id, fieldName: { $in: ["document", "image"] } },
      (err, result) => {
        if (err) {
          throw err;
        }
...
const findDocs = async (userID) => {
  let documents;
  await Document.find(
    { user_id: userID, fieldName: { $in: ["document", "image"] } },
    (err, result) => {
      if (err) {
        throw err;
      }
...
const displayPicture = async (req, res, next) => {
  try {
    await Document.findOne(
      { _id: req.params.id, fieldName: "image" },
      (err, doc) => {
        if (err) {
          throw err;
        }
...
const displayProfilePic = async (req, res) => {
  try {
    await Document.findOne(
      { _id: req.user.id, fieldName: "profile-pic" },
      (err, doc) => {
        if (err) {
          throw err;
        }
...
const deleteExclusiveFile = async (userID, fieldName) => {
  let deleteStatus;
  await Document.findOne(
    { user_id: userID, fieldName: fieldName },
    (err, doc) => {
      if (err) {
        console.log("Error found");
        throw err;
  ...
  try {
    ...
    if (deleteStatus) {
      ...
      res
        .status(400)
        .json(
          "Profile picture not deleted or there was no profile picture to delete"
        );
    }
  } catch (error) {
