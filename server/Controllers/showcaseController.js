  ...
  await Showcase.findOne({
    ...
    const createFeaturedWork = async (req, res) => {
      ...
      await FeaturedWork.findOne(
        { user_id: req.user.id, title: req.body.title },
        (err, result) => {
          if (err) {
            throw err;
          }
          if (result) {
            res
              .status(400)
              .json('A project with the same title already exists');
            return;
          } else {
            if (req.files) {
              (async () => {
    ...
    const addFiles = async (req, res) => {
      try {
        if (req.files) {
          ...
          await FeaturedWork.findOneAndUpdate(
...
const addFiles = async (req, res) => {
  try {
    await FeaturedWork.findOneAndUpdate(
      { _id: req.params.id, user_id: req.user.id },
      { $addToSet: { attachedFiles: req.body.attachedFiles } },
      { new: true },
      (err, result) => {
        if (err) {
          throw err;
...
const clearShowcase = async (req, res) => {
  try {
    ...
    res
      .status(400)
      .json(
        "An error occured while trying to delete the user's featured works"
      );
  }
};
