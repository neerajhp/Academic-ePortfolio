...
test("Should add work experience to database", async () => {
  await request
    ...
    .send({
      ...
      description:
        "Striker who scored 20 goals a season, and won the golden boot in 2012",
    })
    .expect(200)
    .then((data) => {
      expect(data).toBeDefined();
...
test("Should update the specified experience object", async () => {
  await request
    .put(`/api/experience/edit/${objectID}`)
    .set("Cookie", "token=" + token)
    .send({
...
test("Should get all experience as a viewer", async () => {
  await request
    .get(`/api/view/experience/${userName}`)
    .set("Cookie", "token=" + token)
    .expect(200)
    .then((data) => {
      expect(data.body).toBeDefined();
      expect(data.body.extracurricular[0]).toEqual(
        expect.objectContaining({
          type: "extracurricular",
          organization: "Soccer club",
          role: "Member",
      ...
      expect(data.body.employment[0]).toEqual(
        expect.objectContaining({
          type: "employment",
          organization: "FC Barcelona",
          role: "Manager",
      ...
      expect(data.body.volunteering[0]).toEqual(
        expect.objectContaining({
          type: "volunteering",
          organization: "UNICEF",
          role: "Admin",
...
test("Should delete the specified experience object", async () => {
  await request
    .delete(`/api/experience/delete/${objectID}`)
    .set("Cookie", "token=" + token)
    .expect(200)
    .then((data) => {
      expect(data.body).toBeDefined();
